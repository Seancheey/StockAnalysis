...
from database.sql_tables import ExchangeMarkets, CNStocks, DailyPriceSummaries
...
def retrieve_price():
    ...
    exchange_df = unique_of(transform(df, [
        Pipeline('ts_code', CNStocks.exchange.key, lambda x: x.split('.')[-1]),
    ]), CNStocks.exchange.key)
    insert_dataframe(exchange_df, ExchangeMarkets, duplicate_update=False)

    # make sure stock dependency met
    stocks_df = transform(df, [
        Pipeline('ts_code', CNStocks.exchange.key, lambda x: x.split('.')[-1]),
        Pipeline('ts_code', CNStocks.code.key, lambda x: x.split('.')[0]),
    ])
    insert_dataframe(stocks_df, CNStocks, duplicate_update=False)
